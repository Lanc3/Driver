...
camera::camera(glm::vec3 position, glm::vec3 up) : Front(glm::vec3(0.0f, 0.0f, -1.0f)),Yaw(YAW),Pitch(PITCH), MovementSpeed(SPEED), MouseSensitivity(SENSITIVTY), Zoom(ZOOM)
{
...
void camera::ProcessInput(float dt, unsigned char key, GLfloat xoffset, GLfloat yoffset)
{
	if (key == 'w')
	{
